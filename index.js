...
if (message.content === '!quiz') {
    ...
    if (message.content === '!quiz') {
        ...
        collector.on('collect', async collected => {
            ...
            if (userAnswer - 1 === quiz.answer) {
                await message.channel.send(`✅ Correct, ${message.author}! You earned **20 XP**!`);
                ...
                if (newLevel > user.level) {
                    user.level = newLevel;
                    await message.channel.send(`🎉 Level up! ${message.author} reached level ${newLevel}!`);
                }
...
//Start DM message 
client.on('messageCreate', async (message) => {
    if (message.channel.id === process.env.CONSOLE_CHANNEL_ID && message.author.bot) {
        if (message.content.includes('Done') || message.content.includes('Server started')) {
            try {
                const owner = await client.users.fetch(OWNER_ID);
                await owner.send('✅ Your Minecraft server has started successfully!');
                console.log('✅ Sent DM to owner about server start.');
            } catch (error) {
